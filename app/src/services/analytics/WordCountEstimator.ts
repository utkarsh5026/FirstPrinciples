export class WordCountEstimator {
  ...
  public countWords(text: string): number {
    ...
    const cleanText = this.removeMarkdownFormatting(text);
  ...
  public estimateReadingTime(
    wordCount: number,
    readingSpeed = WordCountEstimator.AVERAGE_READING_SPEED_WPM
  ): number {
   ...
   */
  public estimateReadingProgress(
    wordCount: number,
    timeSpentMs: number,
    readingSpeed = WordCountEstimator.AVERAGE_READING_SPEED_WPM
  ): number {
    ...
    const totalReadingTimeMs = this.estimateReadingTime(
      wordCount,
      readingSpeed
    );
...
export const wordCountEstimator = new WordCountEstimator();
